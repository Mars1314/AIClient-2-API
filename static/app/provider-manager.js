...
function renderProviders(providers) {
    ...
    // 只显示 claude-kiro-oauth 提供商
    const providerType = 'claude-kiro-oauth';

    // 检查是否有该提供商的数据
    const hasProviders = providers && providers[providerType];
    ...
    // 计算总统计
    const totalAccounts = totalCount;
    const totalHealthy = healthyCount;
    ...
    // 更新统计卡片数据 - 只有一个提供商类型
    const activeProviders = totalCount > 0 ? 1 : 0;
    updateProviderStatsDisplay(activeProviders, totalHealthy, totalCount);
}
